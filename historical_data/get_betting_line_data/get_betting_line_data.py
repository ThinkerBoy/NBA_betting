...
def main():
    ...
    ## todays_date = '20120930'
    years = [2012, 2013, 2014, 2015, 2016]
    for year in years:
        df = pd.read_csv(os.path.join(os.path.dirname(os.path.abspath(__file__)),'../get_schedule_data/',str(year),'games.csv'), sep="|")
        dates = df['date'].unique()
        dates.sort()

        for date in dates:
            todays_date = date.replace('-','')
            print(todays_date)
            ...
            all_lines = pd.concat([all_lines, write_df])
            print(all_lines.shape)
            
    with open(os.path.join(os.getcwd(),'SBR_NBA_Lines.csv'), 'a') as f:
        all_lines.to_csv(f, sep="|", index=False)#, header = False)
